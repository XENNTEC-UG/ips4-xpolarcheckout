...
| --- | --- | --- |
| `1.0.17` | `10017` | `app-source/data/application.json` |
...
| Dynamic product mapping | On-demand Polar product creation mirroring IPS packages | Correct product names on Polar receipts and emails |
| Rich invoice view (Stripe parity) | Two-column settlement display with charge summary, payment refs, and status badges | Customer-facing transparency matching Stripe-level invoice detail |
| Polar invoice generation | Automatic invoice creation on Polar via API after payment | Customers get hosted invoice pages and PDF downloads |
| Enriched settlement snapshots | 13+ additional fields from Polar Order model (discount, billing, customer, line items) | Complete audit trail with full provider data |
| IPS coupon forwarding | Forward IPS Nexus coupons as one-time Polar discounts with math verification | Proper discount breakdown on Polar hosted checkout |
| Discount code control | ACP toggle for Polar-side promotional codes with automatic lockout when IPS coupon active | Prevents double-discounting |
| ACP member profile block | Transaction count, refund count, and last activity summary on ACP member profiles | Quick member payment health at a glance |
...
- Polar event map: `order.created`, `order.paid`, `order.updated`, `order.refunded`, `refund.created`, `refund.updated`, `checkout.updated` with safe state transition helpers and terminal-state guardrails.
...
- Dynamic Polar product mapping: on-demand product creation mirroring IPS Nexus packages with cached lookup table and name sync at checkout time.
- ACP product mapping viewer with bulk name sync capability.
- ACP checkout-flow controls:
  - `Allow Polar for all carts`
  - `Hybrid route: show Polar only for single-item carts` (Polar hidden for multi-item invoices during method selection).
- Multi-item consolidated label modes for Polar receipts/invoices:
  - legacy first-item label
  - neutral `Invoice # + item count`
  - explicit item-list label (`Item A + Item B + ...`) with safe truncation.

- Rich invoice view with Stripe-parity two-column layout: Charge Summary (subtotal, discount, tax, total, refunded, status badge) + Payment & References (order/checkout IDs, invoice/PDF/receipt links, captured timestamp).
- Global order details enhancements: products subtotal before coupons, coupon tag icons, green discount styling, duplicate coupon section hiding.
- Clients page settlement hook with provider total + IPS total comparison.
- Print invoice hook with charge summary and payment references tables.
- Idempotency guard for dual-app installations (Stripe + Polar) to prevent double-processing of global enhancements.

- Polar invoice generation: automatic `POST /v1/orders/{id}/invoice` trigger after `order.paid`, followed by URL fetch on `order.updated` with `is_invoice_generated`.
- Enriched settlement snapshots: 13+ additional fields from Polar Order model (discount, net amount, refunded tax, invoice number, billing name/reason, customer email/name/tax IDs, discount name/code, line items).
- Invoice URL persistence across snapshot rebuilds prevents later events from clearing previously-fetched URLs.

- IPS coupon forwarding: detects negative-amount invoice items (coupons, gateway discounts), creates one-time Polar discounts via `POST /v1/discounts`, attaches to checkout session with math verification and graceful fallback.
- Discount code control: ACP toggle for Polar-side promotional codes (default OFF). When IPS coupon is active, Polar codes are always blocked regardless of toggle.

### Post-MVP targets
- Expanded analytics cards in integrity panel.
- Additional alert channels for persistent payment integrity faults.
- Tax ID collection and tax readiness monitoring (lang keys and method stubs scaffolded, not yet wired into checkout flow).
- Dispute and chargeback tracking via Polar API (lang keys defined, data fetching not yet implemented).
...
- Optional hybrid checkout gating is enforced in both:
  - `checkValidity()` (hides method for multi-item invoices in checkout UI), and
  - `auth()` (defense-in-depth guard if gateway is forced).
- Consolidated multi-item receipts can use reusable synthetic Polar products so receipt titles remain explicit while preserving one-payment checkout constraints.
...
- ACP integrity panel with environment badge (sandbox/production), replay status, health indicators, and acknowledge/delete error controls.
...
- ACP member profile block showing per-member transaction count, refund count, and last activity timestamp.
