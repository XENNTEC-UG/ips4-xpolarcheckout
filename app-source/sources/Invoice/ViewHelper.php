<?php
/**
 * @brief		Invoice View Helper — extracted logic for invoiceViewHook
 * @author		XENNTEC UG
 * @copyright	(c) XENNTEC UG
 * @package		IPS Community Suite
 * @subpackage	X Polar Checkout
 * @since		25 Mar 2026
 */

namespace IPS\xpolarcheckout\Invoice;
 ...
 * Invoice View Helper
 *
 * Centralises the HTML-building logic that the thin invoiceViewHook delegates to.
 * Every public method is a pure function: input in, string out, no side effects.
 */
class _ViewHelper
{
	/**
	 * Constructor stub (required for IPS4 dev-mode compliance — at least one non-static method)
	 */
	 ...
	 * @return	string
	 */
	public static function buildPolarSummary( array $snapshot )
	{
	 ...
	 * @return	string
	 */
	public static function buildPaymentRefs( array $snapshot )
	{
		$lang = \IPS\Member::loggedIn()->language();
		$html = '';
	 ...
	 * @return	string
	 */
	public static function wrapInColumns( $output, $polarSummary )
	{
		...
		$boxEnd = static::findClosingDiv( $output, $boxStart );
		if ( $boxEnd === false )
		{
			return $output;
		}
	 ...
	 */
	public static function findClosingDiv( $html, $startPos )
	{
		$depth = 0;
		$len = mb_strlen( $html );
		$i = $startPos;
	 ...
	 * @return	string
	 */
	public static function insertPaymentRefs( $output, $paymentRefs )
	{
	 ...
	 * @return	string
	 */
	public static function enhanceOrderDetails( $output, \IPS\nexus\Invoice $invoice )
	{
		...
		$couponNames = array();
		foreach ( clone $invoice->items as $item )
		{
		...
		$productsSubtotal = static::productsSubtotal( $invoice );
	 ...
	 */
	public static function productsSubtotal( \IPS\nexus\Invoice $invoice )
	{
		$hasCoupon = false;
		...
		return new \IPS\nexus\Money( $productsTotal, $invoice->currency );
	}
}
